...
int main(int argc, char* argv[])
{
    ...
    laserGrid->info.resolution = 0.2;
    laserGrid->info.width = 500;
    laserGrid->info.height = 500;
    ...
    ros::Subscriber laserSub = nh.subscribe("base_scan", 1, laserCallback);
    
    ros::Publisher laserMapPub_ = nh.advertise<nav_msgs::OccupancyGrid>("laserMap", 0);
    laserMapPub = &laserMapPub_;
    ros::Publisher pathPub = nh.advertise<nav_msgs::Path>("path", 1);
    
    tfListener = new tf::TransformListener();
    
    Pathfinder pather(nh);
    pather.SetTarget(20.0, 84.0);
