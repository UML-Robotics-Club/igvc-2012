...
GZ_REGISTER_DYNAMIC_CONTROLLER("diffdrive_quad", DiffDriveQuad);

enum
{
    RIGHT, LEFT, RRIGHT, RLEFT
};

// Constructor
DiffDriveQuad::DiffDriveQuad(Entity *parent) :
    Controller(parent) //, debug("/tmp/diffdrive_quad.log")
{
  ...
  leftRearJointNameP = new ParamT<std::string> ("leftRearJoint", "", 1);
  rightRearJointNameP = new ParamT<std::string> ("rightRearJoint", "", 1);
...
// Destructor
DiffDriveQuad::~DiffDriveQuad()
{
  delete leftJointNameP;
  delete rightJointNameP;
  delete leftRearJointNameP;
  delete rightRearJointNameP;
  delete wheelSepP;
  delete wheelDiamP;
  delete torqueP;
...
// Load the controller
void DiffDriveQuad::LoadChild(XMLConfigNode *node)
{
  ...
  leftRearJointNameP->Load(node);
  rightRearJointNameP->Load(node);

  joints[LEFT]   = parent_->GetJoint(**leftJointNameP);
  joints[RIGHT]  = parent_->GetJoint(**rightJointNameP);
  joints[RLEFT]  = parent_->GetJoint(**leftRearJointNameP);
  joints[RRIGHT] = parent_->GetJoint(**rightRearJointNameP);
  ...
  if (!joints[RLEFT])
    gzthrow("The controller couldn't get right rear hinge joint");
  ...
  int argc = 0;
  char** argv = NULL;
  ros::init(argc, argv, "diffdrive_quad", 
  ...
  ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(topicName, 1,
          boost::bind(&DiffDriveQuad::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
...
void DiffDriveQuad::InitChild()
{
  ...
  callback_queue_thread_ = new boost::thread(boost::bind(&DiffDriveQuad::QueueThread, this));
}

// Load the controller
void DiffDriveQuad::SaveChild(std::string &prefix, std::ostream &stream)
{
  stream << prefix << *(leftJointNameP) << "\n";
  stream << prefix << *(rightJointNameP) << "\n";
  stream << prefix << *(leftRearJointNameP) << "\n";
  stream << prefix << *(rightRearJointNameP) << "\n";
...
void DiffDriveQuad::UpdateChild()
{
  ...
  if (enableMotors)
  {
    ...
    joints[RLEFT]->SetVelocity(0, wheelSpeed[LEFT] / (**(wheelDiamP) / 2.0));
    joints[RRIGHT]->SetVelocity(0, wheelSpeed[RIGHT] / (**(wheelDiamP) / 2.0));

    joints[LEFT]->SetMaxForce(0, **(torqueP));
    joints[RIGHT]->SetMaxForce(0, **(torqueP));
    joints[RLEFT]->SetMaxForce(0, **(torqueP));
    joints[RRIGHT]->SetMaxForce(0, **(torqueP));
  }
...
void DiffDriveQuad::GetPositionCmd()
{
  lock.lock();
  ...
  //std::cout << "Wheel speeds: " << wheelSpeed[LEFT] << " " << wheelSpeed[RIGHT] << std::endl;

  lock.unlock();
}

// NEW: Store the velocities from the ROS message
void DiffDriveQuad::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_msg)
{
...
void DiffDriveQuad::QueueThread()
{
...
void DiffDriveQuad::publish_odometry()
{
  // get current time
  ros::Time current_time_((Simulator::Instance()->GetSimTime()).sec, 
      (Simulator::Instance()->GetSimTime()).nsec); 
  ...
  gazebo::Quatern rot;
  rot.SetFromEuler(gazebo::Vector3(pos_iface_->data->pose.roll, pos_iface_->data->pose.pitch, 
...
void DiffDriveQuad::write_position_data()
{
  // TODO: Data timestamp
